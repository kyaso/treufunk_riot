...
int change_state(int argc, char **argv)
{
    ...
    devs[0].state = state;
    printf("Simulating automatic state change to state %d\n", state);
    return 0;
}

int change_fifo(int argc, char **argv)
{
    uint8_t fifo_empty = atoi(argv[1]);
    devs[0].fifo_empty = fifo_empty;
    printf("FIFO_empty flag now set to %d\n", fifo_empty);
    return 0;
}
                ...
                char *data)
{
    ...
    if (((unsigned)iface) > (TREUFUNK_NUM - 1)) {
