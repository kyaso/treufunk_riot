...
void treufunk_reset(treufunk_t *dev)
{
    ...
    treufunk_reg_write(dev, RG_GLOBAL_RESETB,   0x00);
    treufunk_reg_write(dev, RG_GLOBAL_RESETB,   0xFF);
    treufunk_reg_write(dev, RG_GLOBAL_initALL,  0xFF);

    /* Clock reference */
    DEBUG("Setting CLK references...\n");
    treufunk_sub_reg_write(dev, SR_CTRL_CLK_CDE_OSC,    0);
    treufunk_sub_reg_write(dev, SR_CTRL_CLK_CDE_PAD,    1);
    treufunk_sub_reg_write(dev, SR_CTRL_CLK_DIG_OSC,    0);
    treufunk_sub_reg_write(dev, SR_CTRL_CLK_DIG_PAD,    1);
    treufunk_sub_reg_write(dev, SR_CTRL_CLK_PLL_OSC,    0);
    treufunk_sub_reg_write(dev, SR_CTRL_CLK_PLL_PAD,    1);
    treufunk_sub_reg_write(dev, SR_CTRL_CLK_C3X_OSC,    0);
	treufunk_sub_reg_write(dev, SR_CTRL_CLK_C3X_PAD,    1);
    treufunk_sub_reg_write(dev, SR_CTRL_CLK_FALLB,      0);

    /* ADC_CLK */
    DEBUG("Setting ADC_CLK...\n");
    treufunk_sub_reg_write(dev, SR_CTRL_CDE_ENABLE,    0);
	treufunk_sub_reg_write(dev, SR_CTRL_C3X_ENABLE,    1);
	treufunk_sub_reg_write(dev, SR_CTRL_CLK_ADC,       1);
    treufunk_sub_reg_write(dev, SR_CTRL_C3X_LTUNE,     1);

    /* LDOs */
    DEBUG("Configuring LDOs...\n");
    treufunk_sub_reg_write(dev, SR_LDO_A_VOUT,      21);
	treufunk_sub_reg_write(dev, SR_LDO_D_VOUT,      24);
	treufunk_sub_reg_write(dev, SR_LDO_PLL_VOUT,    24);
    treufunk_sub_reg_write(dev, SR_LDO_VCO_VOUT,    24);
    treufunk_sub_reg_write(dev, SR_LDO_TX24_VOUT,   23);

    /* PLL config */
    DEBUG("Configuring PLL...\n");
    treufunk_sub_reg_write(dev, SR_IREF_PLL_CTRLB,   0);
	treufunk_sub_reg_write(dev, SR_PLL_VCO_TUNE,   235); /* Channel 12 */
    treufunk_sub_reg_write(dev, SR_PLL_LPF_C,        0);
    treufunk_sub_reg_write(dev, SR_PLL_LPF_R,        9);

    /* Enable 2.4GHz band */
    DEBUG("Enabling 2.4 GHz band...\n");
    treufunk_sub_reg_write(dev, SR_RX_RF_MODE,     0);
	treufunk_sub_reg_write(dev, SR_RX_LO_EXT,      0);
	treufunk_sub_reg_write(dev, SR_LNA24_ISETT,    7);
    treufunk_sub_reg_write(dev, SR_LNA24_SPCTRIM, 15);

    /* ADC Settings */
    DEBUG("Configuring ADC...\n");
    treufunk_sub_reg_write(dev, SR_CTRL_ADC_MULTIBIT, 0);
	treufunk_sub_reg_write(dev, SR_CTRL_ADC_ENABLE,   1);
	treufunk_sub_reg_write(dev, SR_CTRL_ADC_BW_SEL,   1);
	treufunk_sub_reg_write(dev, SR_CTRL_ADC_BW_TUNE,  5);
    treufunk_sub_reg_write(dev, SR_CTRL_ADC_DR_SEL,   2);

    /* Polyphase filter settings */
    DEBUG("Configuring PPF...\n");
    treufunk_sub_reg_write(dev, SR_PPF_M0,    0);
    treufunk_sub_reg_write(dev, SR_PPF_M1,    0);
	treufunk_sub_reg_write(dev, SR_PPF_TRIM,  0);
	treufunk_sub_reg_write(dev, SR_PPF_HGAIN, 1);
    treufunk_sub_reg_write(dev, SR_PPF_LLIF,  0);

    /* Demodulator settings */
    DEBUG("Configuring demodulator...\n");
    treufunk_sub_reg_write(dev, SR_DEM_CLK96_SEL,          1);
	treufunk_sub_reg_write(dev, SR_DEM_AGC_EN,             1);
    treufunk_sub_reg_write(dev, SR_DEM_FREQ_OFFSET_CAL_EN, 0);
	treufunk_sub_reg_write(dev, SR_DEM_OSR_SEL,            0);
	treufunk_sub_reg_write(dev, SR_DEM_BTLE_MODE,          1); /* 0xAA BLE preamble */
	treufunk_sub_reg_write(dev, SR_DEM_IF_SEL,             2); /* 1 MHz */
	treufunk_sub_reg_write(dev, SR_DEM_DATA_RATE_SEL,      3); /* 2 Mbits */
	treufunk_sub_reg_write(dev, SR_DEM_IQ_CROSS,           1);
    treufunk_sub_reg_write(dev, SR_DEM_IQ_INV,             0);

    /* initial CIC filter gain settings */
    DEBUG("Configuring CIC filter...\n");
    treufunk_sub_reg_write(dev, SR_DEM_GC1, 0);
	treufunk_sub_reg_write(dev, SR_DEM_GC2, 0);
	treufunk_sub_reg_write(dev, SR_DEM_GC3, 1);
	treufunk_sub_reg_write(dev, SR_DEM_GC4, 0);
	treufunk_sub_reg_write(dev, SR_DEM_GC5, 0);
	treufunk_sub_reg_write(dev, SR_DEM_GC6, 1);
    treufunk_sub_reg_write(dev, SR_DEM_GC7, 4);

    /* General TX settings */
    DEBUG("Configuring TX path...\n");
    treufunk_sub_reg_write(dev, SR_PLL_MOD_DATA_RATE,    2); /* 1 Mbit */
	treufunk_sub_reg_write(dev, SR_PLL_MOD_FREQ_DEV,    21); /* 500 kHz Freqdev */
	treufunk_sub_reg_write(dev, SR_TX_EN,                1);
	treufunk_sub_reg_write(dev, SR_TX_ON_CHIP_MOD,       1);
	treufunk_sub_reg_write(dev, SR_TX_UPS,               0);
	treufunk_sub_reg_write(dev, SR_TX_ON_CHIP_MOD_SP,    1); /* TX uses 1Mbit/s datarate */
	treufunk_sub_reg_write(dev, SR_TX_AMPLI_OUT_MAN_H,   1);
    treufunk_sub_reg_write(dev, SR_TX_AMPLI_OUT_MAN_L, 255);
    ...
    treufunk_sub_reg_write(dev, SR_FIFO_MODE_EN,    1);
	treufunk_sub_reg_write(dev, SR_WAKEUPONSPI,     1);
	treufunk_sub_reg_write(dev, SR_WAKEUPONRX,      0);
    treufunk_sub_reg_write(dev, SR_WAKEUP_MODES_EN, 0);

    /* Startup counter Settings */
    DEBUG("Setting up startup counter...\n");
	treufunk_reg_write(dev, RG_SM_TIME_POWER_TX, 0xff);
	treufunk_reg_write(dev, RG_SM_TIME_POWER_RX, 0xff);
	treufunk_reg_write(dev, RG_SM_TIME_PLL_PON,  0xff);
	treufunk_reg_write(dev, RG_SM_TIME_PLL_SET,  0xff);
	treufunk_reg_write(dev, RG_SM_TIME_TX,       0xff);
    treufunk_reg_write(dev, RG_SM_TIME_PD_EN,    0xff);

    /* SM TX */
    DEBUG("configuring SM TX...\n");
	treufunk_sub_reg_write(dev, SR_TX_MODE,          0));
	treufunk_sub_reg_write(dev, SR_INVERT_FIFO_CLK,  0));
	treufunk_sub_reg_write(dev, SR_DIRECT_RX,        1)); /* Change directly to RX after completed transmission (TX) */
	treufunk_sub_reg_write(dev, SR_TX_ON_FIFO_IDLE,  0));
	treufunk_sub_reg_write(dev, SR_TX_ON_FIFO_SLEEP, 0));
	treufunk_sub_reg_write(dev, SR_TX_IDLE_MODE_EN,  0));
	treufunk_sub_reg_write(dev, SR_TX_PWR_CTRL,     15)); /* Maximum TX output power */
    treufunk_sub_reg_write(dev, SR_TX_MAXAMP,        0));

    /* SM RX */
    DEBUG("Configuring SM RX...\n");
	treufunk_sub_reg_write(dev, SR_DIRECT_TX,          0);
	treufunk_sub_reg_write(dev, SR_DIRECT_TX_IDLE,     0);
	treufunk_sub_reg_write(dev, SR_RX_HOLD_MODE_EN,    0);
	treufunk_sub_reg_write(dev, SR_RX_TIMEOUT_EN,      0);
	treufunk_sub_reg_write(dev, SR_RX_HOLD_ON_TIMEOUT, 0);
    treufunk_sub_reg_write(dev, SR_AGC_AUTO_GAIN,      0);
	...
	treufunk_sub_reg_write(dev, SR_RX_LENGTH_H, 0x00);
	treufunk_sub_reg_write(dev, SR_RX_LENGTH_M, 0x43);
	treufunk_sub_reg_write(dev, SR_RX_LENGTH_L, 0xC0);

    /* Timeout counter */
    /* We don't use the timeout counter, so set all bits to 1 */
    DEBUG("Setting up timeout counter...\n");
	treufunk_sub_reg_write(dev, SR_RX_TIMEOUT_H, 0xFF);
	treufunk_sub_reg_write(dev, SR_RX_TIMEOUT_M, 0xFF);
    treufunk_sub_reg_write(dev, SR_RX_TIMEOUT_L, 0xFF);

    /* Resets */
    DEBUG("Doing resets...\n");
	 /* TODO (treufunk_reset): Call function reset_fifo() or leave this ? */
	treufunk_sub_reg_write(dev, SR_SM_EN,       1);
